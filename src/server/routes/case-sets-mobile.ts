...
// DELETE /api/case-sets-mobile/:id
...
// POST /api/case-sets-mobile/:id/execute
caseSetRoutes.post('/:id/execute', async (req: Request, res: Response) => {
  ...
  try {
    const result = await runCaseSet(set.id, 'mobile', executor, filterCaseIds, environmentId, deviceId);
    res.json({
      code: 200,
      message: 'ok',
      data: {
    ...
    console.error(`[routes:case-sets-mobile] execute failed setId=${set.id} err="${msg}"`);
    res.status(500).json({ code: 500, message: msg });
  }
});

// GET /api/case-sets-mobile/:id/executions
...
// GET /api/case-sets-mobile/:id/executions/:eid
caseSetRoutes.get('/:id/executions/:eid', (req: Request, res: Response) => {
  ...
  const items = findCaseSetExecutionItems(eid).map((it) => ({
    ...
    report_url: it.report_path && it.case_execution_id
      ? relativeReportUrl('mobile', it.case_id, it.case_execution_id)
      : null,
  }));
