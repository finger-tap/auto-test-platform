/**
 * /api/schedule-sets-pc — 调度 PC 用例集
 */
...
} from '../db/schedule-sets-pc.js';
import { findSetById } from '../db/case-sets-pc.js';
import {
  pauseSchedule,
  resumeSchedule,
  unscheduleCronJob,
...
export const scheduleSetPcRoutes = Router();
scheduleSetPcRoutes.use(authMiddleware);

// GET /api/schedule-sets-pc
scheduleSetPcRoutes.get('/', (req: Request, res: Response) => {
...
// GET /api/schedule-sets-pc/:id
scheduleSetPcRoutes.get('/:id', (req: Request, res: Response) => {
...
// PUT /api/schedule-sets-pc/set/:setId — upsert schedule for a case set
scheduleSetPcRoutes.put('/set/:setId', (req: Request, res: Response) => {
...
// PUT /api/schedule-sets-pc/:id/configure
scheduleSetPcRoutes.put('/:id/configure', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const schedule = findScheduleSetById(id);
  if (!schedule) {
    res.status(404).json({ code: 404, message: 'Schedule not found' });
...
// POST /api/schedule-sets-pc/:id/pause
scheduleSetPcRoutes.post('/:id/pause', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const schedule = findScheduleSetById(id);
...
// POST /api/schedule-sets-pc/:id/resume
scheduleSetPcRoutes.post('/:id/resume', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const schedule = findScheduleSetById(id);
  if (!schedule) { res.status(404).json({ code: 404, message: 'Schedule not found' }); return; }
  const set = findSetById(schedule.case_set_id);
  ...
  resumeSchedule(id, 'pc');
  res.json({ code: 200, message: 'Resumed', data: findScheduleSetById(id) });
});

// POST /api/schedule-sets-pc/:id/remove
scheduleSetPcRoutes.post('/:id/remove', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const schedule = findScheduleSetById(id);
  if (!schedule) { res.status(404).json({ code: 404, message: 'Schedule not found' }); return; }
  const set = findSetById(schedule.case_set_id);
