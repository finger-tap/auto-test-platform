...
export const tScenarioSets = mysqlTable('t_scenario_sets', {
  ...
  name: varchar('name', { length: 255 }).notNull(),
  description: text('description'),
  scenario_ids: text('scenario_ids').notNull().default('[]'),
  ...ts(),
...
function caseSetTable(name: string) {
  return mysqlTable(name, {
    ...
    name: varchar('name', { length: 255 }).notNull(),
    description: text('description'),
    test_case_ids: text('test_case_ids').notNull().default('[]'),
    tags: text('tags'),
    status: varchar('status', { length: 32 }).notNull().default('active'),
    ...ts(),
...
export const tEnvironments = mysqlTable('t_environments', {
  ...
  ssl_certs: text('ssl_certs').notNull().default('[]'),
...
function mockTable(name: string) {
  return mysqlTable(name, {
    ...
    response_headers: text('response_headers').default('{"Content-Type":"application/json"}'),
    response_body: mediumtext('response_body').default('{}'),
...
export const tNotifyChannels = mysqlTable('t_notify_channels', {
  ...
  secret: text('secret'),
  events: text('events').notNull().default('["resource.update","resource.delete"]'),
  enabled: int('enabled').notNull().default(1),
