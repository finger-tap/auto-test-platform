...
export function findSetsByUserIdPaginated(
  ...
  const rows = db.prepare(
    `SELECT * FROM case_sets_pc WHERE ${where} ORDER BY ${sortCol} ${sortDir} LIMIT ? OFFSET ?`
  ).all(...params, pageSize, offset) as CaseSetRow[];

  const { count } = db.prepare(
    `SELECT COUNT(*) AS count FROM case_sets_pc WHERE ${where}`
  ...
  const items = rows.map(r => {
    ...
    const allItems = db.prepare(`
      ...
      FROM case_set_execution_items_pc sei
      JOIN case_set_executions_pc sse ON sei.set_execution_id = sse.id
      WHERE sse.set_id = ?
...
export function findSetsByUserId(userId: number): CaseSetItem[] {
  const rows = db.prepare('SELECT * FROM case_sets_pc WHERE user_id = ? ORDER BY updated_at DESC').all(userId) as CaseSetRow[];
  return rows.map((r) => {
    let count = 0;
    try {
      const ids = JSON.parse(r.test_case_ids || '[]');
...
export function findSetById(id: number): CaseSetRow | undefined {
  return db.prepare('SELECT * FROM case_sets_pc WHERE id = ?').get(id) as CaseSetRow | undefined;
}

export function findSetByName(userId: number, name: string): CaseSetRow | undefined {
  return db.prepare('SELECT * FROM case_sets_pc WHERE user_id = ? AND name = ?').get(userId, name) as CaseSetRow | undefined;
}
...
export function createSet(userId: number, name: string, description?: string, testCaseIds?: number[], tags?: string, status?: string): number {
  ...
  const result = db.prepare(
    `INSERT INTO case_sets_pc (user_id, name, description, test_case_ids, tags, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
...
export function updateSet(id: number, userId: number, data: {
  ...
  const result = db.prepare(`UPDATE case_sets_pc SET ${fields.join(', ')} WHERE id = ? AND user_id = ?`).run(...vals);
  return result.changes > 0;
}

export function deleteSet(id: number, userId: number): boolean {
  const result = db.prepare('DELETE FROM case_sets_pc WHERE id = ? AND user_id = ?').run(id, userId);
  return result.changes > 0;
}
...
export function createCaseSetExecution(setId: number, triggerType: string, executedBy: string, startedAt: string): number {
  const result = db.prepare(
    `INSERT INTO case_set_executions_pc (set_id, status, trigger_type, executed_by, started_at, finished_at)
     VALUES (?, 'running', ?, ?, ?, '')`
...
export function updateCaseSetExecution(id: number, patch: {
  ...
  if (fields.length === 0) return false;
  vals.push(id);
  const result = db.prepare(`UPDATE case_set_executions_pc SET ${fields.join(', ')} WHERE id = ?`).run(...vals);
  return result.changes > 0;
}

export function findCaseSetExecutions(setId: number): CaseSetExecutionRow[] {
  return db.prepare(
    `SELECT * FROM case_set_executions_pc WHERE set_id = ? ORDER BY started_at DESC`
  ).all(setId) as CaseSetExecutionRow[];
}

export function findCaseSetExecutionById(id: number): CaseSetExecutionRow | undefined {
  return db.prepare('SELECT * FROM case_set_executions_pc WHERE id = ?').get(id) as CaseSetExecutionRow | undefined;
}

export function createCaseSetExecutionItem(setExecutionId: number, caseId: number, caseName: string): number {
  const result = db.prepare(
    `INSERT INTO case_set_execution_items_pc (set_execution_id, case_id, case_name, status, started_at, finished_at)
...
export function updateCaseSetExecutionItem(id: number, patch: {
  ...
  if (fields.length === 0) return false;
  vals.push(id);
  const result = db.prepare(`UPDATE case_set_execution_items_pc SET ${fields.join(', ')} WHERE id = ?`).run(...vals);
  return result.changes > 0;
}

export function findCaseSetExecutionItems(setExecutionId: number): CaseSetExecutionItemRow[] {
  return db.prepare(
    `SELECT * FROM case_set_execution_items_pc WHERE set_execution_id = ? ORDER BY id`
