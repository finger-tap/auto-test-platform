/**
 * schedule_sets_pc — 调度 PC 用例集
 * 表名 schedule_sets_pc;FK 指向 case_sets_pc(id)
 */
...
export function findScheduleSetsByUserIdPaginated(
  ...
  const rows = db.prepare(`
    ...
    FROM case_sets_pc rs
    JOIN users u ON u.id = rs.user_id
    LEFT JOIN schedule_sets_pc ss ON ss.case_set_id = rs.id
    WHERE rs.user_id = ?
  ...
  const { count } = db.prepare(
    'SELECT COUNT(*) AS count FROM case_sets_pc WHERE user_id = ?'
...
export function findScheduleSetById(id: number): ScheduleSetRow | undefined {
  return db.prepare('SELECT * FROM schedule_sets_pc WHERE id = ?').get(id) as ScheduleSetRow | undefined;
}

export function findScheduleSetByCaseSetId(caseSetId: number): ScheduleSetRow | undefined {
  return db.prepare('SELECT * FROM schedule_sets_pc WHERE case_set_id = ?').get(caseSetId) as ScheduleSetRow | undefined;
}
...
export function createScheduleSet(data: {
  ...
  const result = db.prepare(
    `INSERT INTO schedule_sets_pc (case_set_id, cron_expr, status, next_run_at)
     VALUES (?, ?, ?, ?)`
  ).run(
    data.case_set_id,
    data.cron_expr ?? null,
...
export function updateScheduleSet(
  ...
  const result = db.prepare(`UPDATE schedule_sets_pc SET ${fields.join(', ')} WHERE id = ?`).run(...values);
...
export function deleteScheduleSet(id: number): boolean {
  const result = db.prepare('DELETE FROM schedule_sets_pc WHERE id = ?').run(id);
  return result.changes > 0;
}

export function findActiveScheduleSets(): ScheduleSetRow[] {
  return db.prepare(`
    SELECT * FROM schedule_sets_pc
