...
export function findScenariosByUserIdPaginated(
  ...
  const items = db.prepare(
    `SELECT * FROM scenarios_mobile WHERE user_id = ? ORDER BY ${sortCol} ${sortDir} LIMIT ? OFFSET ?`
  ).all(userId, pageSize, offset) as ScenarioRow[];
  const { count } = db.prepare(
    `SELECT COUNT(*) AS count FROM scenarios_mobile WHERE user_id = ?`
  ).get(userId) as { count: number };
  return { items, total: count };
}

export function findScenariosByUserId(userId: number): ScenarioRow[] {
  return db.prepare('SELECT * FROM scenarios_mobile WHERE user_id = ? ORDER BY updated_at DESC').all(userId) as ScenarioRow[];
}

export function findScenarioById(id: number): ScenarioRow | undefined {
  return db.prepare('SELECT * FROM scenarios_mobile WHERE id = ?').get(id) as ScenarioRow | undefined;
}
...
export function createScenario(userId: number, data: { name: string; description?: string; status?: string; tags?: string; parameters?: string }): number {
  const result = db.prepare(
    `INSERT INTO scenarios_mobile (user_id, name, description, status, tags, parameters)
     VALUES (?, ?, ?, ?, ?, ?)`
  ).run(
    userId,
    data.name,
...
export function updateScenario(id: number, data: { name?: string; description?: string | null; status?: string; tags?: string; parameters?: string }): boolean {
  ...
  const result = db.prepare(`UPDATE scenarios_mobile SET ${fields.join(', ')} WHERE id = ?`).run(...values);
  return result.changes > 0;
}

export function deleteScenario(id: number): boolean {
  const result = db.prepare('DELETE FROM scenarios_mobile WHERE id = ?').run(id);
  return result.changes > 0;
}

export function findNodesByScenarioId(scenarioId: number): ScenarioNodeRow[] {
  return db.prepare('SELECT * FROM scenario_nodes_mobile WHERE scenario_id = ?').all(scenarioId) as ScenarioNodeRow[];
}
...
export function upsertNodes(scenarioId: number, nodes: Array<{ node_id: string; type: string; position_x: number; position_y: number; label?: string; config?: string }>): void {
  const deleteStmt = db.prepare('DELETE FROM scenario_nodes_mobile WHERE scenario_id = ?');
  const insertStmt = db.prepare(
    `INSERT INTO scenario_nodes_mobile (scenario_id, node_id, type, position_x, position_y, label, config)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  );
...
export function findEdgesByScenarioId(scenarioId: number): ScenarioEdgeRow[] {
  return db.prepare('SELECT * FROM scenario_edges_mobile WHERE scenario_id = ?').all(scenarioId) as ScenarioEdgeRow[];
}
...
export function upsertEdges(scenarioId: number, edges: Array<{ edge_id: string; source_node_id: string; target_node_id: string; source_handle?: string; label?: string }>): void {
  const deleteStmt = db.prepare('DELETE FROM scenario_edges_mobile WHERE scenario_id = ?');
  const insertStmt = db.prepare(
    `INSERT INTO scenario_edges_mobile (scenario_id, edge_id, source_node_id, target_node_id, source_handle, label)
     VALUES (?, ?, ?, ?, ?, ?)`
  );
  const transaction = db.transaction(() => {
    deleteStmt.run(scenarioId);
...
export function createScenarioLog(scenarioId: number, data: { status: string; trigger_type?: string; executed_by?: string; node_results?: string; duration_ms?: number; error_message?: string }): number {
  const result = db.prepare(
    `INSERT INTO scenario_logs_mobile (scenario_id, status, trigger_type, executed_by, node_results, duration_ms, error_message)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(
    scenarioId,
    data.status,
...
export function findLogsByScenarioId(scenarioId: number, limit = 10): ScenarioLogRow[] {
  return db.prepare('SELECT * FROM scenario_logs_mobile WHERE scenario_id = ? ORDER BY executed_at DESC LIMIT ?').all(scenarioId, limit) as ScenarioLogRow[];
}
