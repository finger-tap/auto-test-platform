...
export function findSetsByUserIdPaginated(
  ...
  const rows = db.prepare(
    `SELECT * FROM scenario_sets_mobile WHERE ${where} ORDER BY ${sortCol} ${sortDir} LIMIT ? OFFSET ?`
  ).all(...params, pageSize, offset) as ScenarioSetRow[];

  const { count } = db.prepare(
    `SELECT COUNT(*) AS count FROM scenario_sets_mobile WHERE ${where}`
...
export function findSetsByUserId(userId: number): ScenarioSetItem[] {
  const rows = db.prepare('SELECT * FROM scenario_sets_mobile WHERE user_id = ? ORDER BY updated_at DESC').all(userId) as ScenarioSetRow[];
  return rows.map((r) => {
    let count = 0;
    try {
...
export function findSetById(id: number): ScenarioSetRow | undefined {
  return db.prepare('SELECT * FROM scenario_sets_mobile WHERE id = ?').get(id) as ScenarioSetRow | undefined;
}
...
export function createSet(userId: number, name: string, description?: string, scenarioIds?: number[], tags?: string, status?: string): number {
  ...
  const result = db.prepare(
    `INSERT INTO scenario_sets_mobile (user_id, name, description, scenario_ids, tags, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
...
export function updateSet(id: number, userId: number, data: {
  ...
  const result = db.prepare(`UPDATE scenario_sets_mobile SET ${fields.join(', ')} WHERE id = ? AND user_id = ?`).run(...vals);
  return result.changes > 0;
}

export function deleteSet(id: number, userId: number): boolean {
  const result = db.prepare('DELETE FROM scenario_sets_mobile WHERE id = ? AND user_id = ?').run(id, userId);
  return result.changes > 0;
}
