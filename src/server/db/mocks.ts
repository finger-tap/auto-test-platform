...
export function findMocksByUserIdPaginated(
  ...
  const items = db.prepare(
    'SELECT * FROM mock_endpoints WHERE user_id = ? ORDER BY updated_at DESC LIMIT ? OFFSET ?'
  ).all(userId, pageSize, offset) as MockEndpointRow[];
  const { count } = db.prepare(
    'SELECT COUNT(*) AS count FROM mock_endpoints WHERE user_id = ?'
  ).get(userId) as { count: number };
  return { items, total: count };
}

export function findMockById(id: number): MockEndpointRow | undefined {
  return db.prepare('SELECT * FROM mock_endpoints WHERE id = ?').get(id) as MockEndpointRow | undefined;
}
...
export function createMock(data: {
  ...
  const result = db.prepare(`
    INSERT INTO mock_endpoints
      (user_id, name, method, path_pattern, description, response_status, response_headers, response_body,
...
export function updateMock(id: number, userId: number, data: Partial<{
  name: string;
  method: string;
  path_pattern: string;
  description: string | null;
...
export function deleteMock(id: number, userId: number): boolean {
  const result = db.prepare('DELETE FROM mock_endpoints WHERE id = ? AND user_id = ?').run(id, userId);
  return result.changes > 0;
}
