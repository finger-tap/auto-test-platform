/**
 * schedule_sets_web — 调度 web 用例集
 * 表名 schedule_sets_web;FK 指向 case_sets_web(id)
 */
...
export function findScheduleSetById(id: number): ScheduleSetRow | undefined {
  return db.prepare('SELECT * FROM schedule_sets_web WHERE id = ?').get(id) as ScheduleSetRow | undefined;
}

export function findScheduleSetByCaseSetId(caseSetId: number): ScheduleSetRow | undefined {
  return db.prepare('SELECT * FROM schedule_sets_web WHERE case_set_id = ?').get(caseSetId) as ScheduleSetRow | undefined;
}
...
export function createScheduleSet(data: {
  ...
  const result = db.prepare(
    `INSERT INTO schedule_sets_web (case_set_id, cron_expr, status, next_run_at)
     VALUES (?, ?, ?, ?)`
  ).run(
    data.case_set_id,
    data.cron_expr ?? null,
...
export function updateScheduleSet(
  ...
  const result = db.prepare(`UPDATE schedule_sets_web SET ${fields.join(', ')} WHERE id = ?`).run(...values);
...
export function deleteScheduleSet(id: number): boolean {
  const result = db.prepare('DELETE FROM schedule_sets_web WHERE id = ?').run(id);
  return result.changes > 0;
}

export function findActiveScheduleSets(): ScheduleSetRow[] {
  return db.prepare(`
    SELECT * FROM schedule_sets_web
