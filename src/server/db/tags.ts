...
tagRoutes.get('/', (req: Request, res: Response) => {
  ...
  try {
    const tagMap = new Map<string, { apis: number; scenarios: number; scenario_sets: number }>();

    // 首先从独立的 tags 表获取预定义标签
    const userTags = db.prepare('SELECT name FROM user_tags WHERE user_id = ?').all(userId) as { name: string }[];
    for (const row of userTags) {
      if (!tagMap.has(row.name)) {
        tagMap.set(row.name, { apis: 0, scenarios: 0, scenario_sets: 0 });
      }
    }
    ...
    for (const row of apiRows) {
      if (row.tags) {
        row.tags.split(',').forEach((t) => {
          ...
          if (trimmed) {
            const existing = tagMap.get(trimmed) || { apis: 0, scenarios: 0, scenario_sets: 0 };
    ...
    tagMap.forEach((sources, name) => {
      result.push({ name, count: sources.apis + sources.scenarios + sources.scenario_sets, sources });
    });
...
tagRoutes.post('/', (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existing) {
      res.json({ code: 200, message: 'Tag already exists', data: { name: tagName, exists: true } });
    } else {
      // 插入到 user_tags 表
      const now = new Date().toISOString().replace('T', ' ').substring(0, 19);
      db.prepare('INSERT INTO user_tags (user_id, name, created_at) VALUES (?, ?, ?)').run(userId, tagName, now);
      res.json({ code: 200, message: 'Tag created', data: { name: tagName, exists: false } });
    }
  } catch (err) {
...
// PUT /api/tags/:name/rename - 重命名标签
