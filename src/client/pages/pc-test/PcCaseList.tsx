...
export default function PcCaseList() {
  ...
  const handleDelete = async (e: React.MouseEvent, id: number) => {
    ...
    if (!ok) return;
    const res = await apiFetch(`/pc-cases/${id}`, { method: 'DELETE' });
      ...
      <CollapsibleFilter
        ...
        actions={
          <>
            ...
            <button className="btn btn-primary" onClick={() => navigate('/pc-test/case/new')}>新增</button>
          </>
        }
      />
      ...
      <div className="alist-table-wrap" ref={tableWrapRef}>
        {loading ? (
          ...
          <div className="alist-empty">
            ...
            <button className="sset-btn-create" onClick={() => navigate('/pc-test/case/new')}>+ 新建第一个用例</button>
          </div>
        ) : items.length === 0 ? (
          <div className="alist-empty">
            <p>没有匹配的用例</p>
          ...
          <table className="alist-table">
            ...
            <tbody>
              {items.map((c, index) => (
                <tr key={c.id} className="row-enter" style={{ '--delay': `${index * 30}ms`, cursor: 'pointer' } as React.CSSProperties} onClick={() => navigate(`/pc-test/case/${c.id}`)}>
                  <td className="td-name" style={{ fontWeight: 500 }} title={c.name}>{c.name}</td>
                  <td>
                    <span className={`status-text status-${c.platform}`}>
                      {c.platform === 'mac' ? 'macOS' : c.platform === 'linux' ? 'Linux' : 'Windows'}
                    </span>
                  </td>
                  ...
                  <td>
                    <div className="row-actions">
                      <button className="row-action-btn" title="执行" onClick={e => { e.stopPropagation(); navigate(`/pc-test/case/${c.id}?exec=1`); }}>执行</button>
