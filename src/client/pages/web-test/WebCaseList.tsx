...
export default function WebCaseList() {
  ...
  const handleDelete = (e: React.MouseEvent, id: number) => {
    ...
    apiFetch(`/web-cases/${id}`, { method: 'DELETE' }).then(res => {
      ...
      <div className="alist-filter">
        <div className="alist-filter-row">
          ...
          <div className="alist-actions">
            ...
            <button className="btn btn-primary" onClick={() => navigate('/web-test/case/new')}>+ 新建用例</button>
          </div>
        </div>
      </div>
      ...
      <div className="alist-table-wrap">
        {loading ? (
          ...
          <table className="alist-table">
            ...
            <tbody>
              {items.map(c => (
                <tr key={c.id} onClick={() => navigate(`/web-test/case/${c.id}`)} style={{ cursor: 'pointer' }}>
                  ...
                  <td>
                    <div className="row-actions">
                      <button className="row-action-btn" title="执行" onClick={e => { e.stopPropagation(); navigate(`/web-test/case/${c.id}?exec=1`); }}>▶</button>
