...
export default function MobileTestHome() {
  ...
  useEffect(() => {
    Promise.all([
      apiFetch<{ 'mobile-test': { cases: number; sets: number; passRate: number } }>('/dashboard/stats'),
      apiFetch<{ items: RecentExecution[] }>('/dashboard/recent-executions?limit=20'),
    ]).then(([statsRes, reportsRes]) => {
      const mobileStats = statsRes.data?.['mobile-test'];
      setCaseCount(mobileStats?.cases || 0);
      setSetCount(mobileStats?.sets || 0);
      setPassRate(mobileStats?.passRate || 0);
      setReports((reportsRes.data?.items || []).filter(r => r.test_type === 'mobile').slice(0, 5));
  ...
  const quickActions = [
    { label: '新建用例', path: '/mobile-test/case/new', icon: <path d="M12 5v14M5 12h14" /> },
    { label: '新建用例集', path: '/mobile-test/case-set', icon: <><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M12 8v8M8 12h8" /></> },
    { label: '环境管理', path: '/mobile-test/environment', icon: <><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 01-2.83 2.83l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4" /></> },
    { label: '标签管理', path: '/mobile-test/system-config', icon: <><path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z" /><circle cx="7" cy="7" r="1" /></> },
  ];
          ...
          <table className="dashboard-table">
            ...
            <tbody>
              {reports.map(r => {
                ...
                return (
                  <tr key={r.id} onClick={() => navigate(`/mobile-test/case-set/${r.set_id}`)}>
