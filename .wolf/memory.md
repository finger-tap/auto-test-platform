...
| 26:00 | Phase 5 完成: 执行镜像桥+部署文档; TiDB 18/18 含团队执行; 全部提交 | routes/team-execute.ts, docs/TEAM_DEPLOYMENT.md | ok | ~0 |
| 22:42 | 本地 TiDB playground 停用(DB 迁云);修复 migrate.ts 丢 ?ssl={} 导致 TiDB Cloud 建库连接被拒; .env 改云模板+新增 .env.example; 部署文档补保密规则 | src/server/db-team/migrate.ts, .env, .env.example, docs/TEAM_DEPLOYMENT.md | ok | ~0 |
| 23:05 | 新增 pre-commit 密钥拦截: githooks/pre-commit 拒绝提交 .env*(放行 .env.example), postinstall 自动挂载 core.hooksPath; 实测拦截/放行均通过 | githooks/pre-commit, scripts/setup-git-hooks.mjs, package.json, docs/TEAM_DEPLOYMENT.md, .env.example | ok | ~0 |
| 23:20 | 云 TiDB(62.234.127.90:4000, v8.5.0)连接配置进 .env 并初始化: 建库+2 迁移+31 表全成功; 服务端无 TLS(去掉 ssl={}); 新增 npm run db:init 一键初始化; server 启动路径 teamReady=true 验证通过 | .env, scripts/team-db-init.ts, package.json | ok | ~0 |
| 23:50 | 解答 Grafana 疑问: 用户看的 prometheus-2-0-stats(ID 893)是 Prometheus 自监控面板,与 TiDB 无关; 实查库验证 31 表 0 行 0.2KB(纯 DDL 初始化); 建议切 TiDB-Overview/TiDB 面板看库指标; Grafana API 匿名 401 | 无文件改动 | ok | ~0 |
| 20:15 | 修复 npm run start MODULE_NOT_FOUND: dist/server 从未构建 -> npm run build; 新增 src/server/env.ts 让生产模式也加载 .env; .env 写入 JWT 双密钥; NODE_ENV=production 全链路实测通过(静态产物+云库 teamReady+真密钥) | src/server/index.ts, src/server/env.ts, .env, docs/TEAM_DEPLOYMENT.md | ok | ~0 |
| 22:05 | 邀请码加入团队功能落地: team_invites 表+迁移0002, repo/invites.ts(生成/列表/撤销/兑换), 4 个 API, TeamManageModal 邀请码 tab, RedeemInviteModal+WorkspaceSwitcher 入口; e2e 11 项全过, 测试数据已清 | schema/org.ts, repo/invites.ts, routes/team-org.ts, TeamManageModal.tsx, RedeemInviteModal.tsx, types/team.ts | ok | ~0 |
| 23:30 | 平台管理员+邀请链接模型: center_users.is_platform_admin(迁移0003), 首个注册自动成为平台管理员, TEAM_CREATE_POLICY=admin|self(默认admin仅管理员建团), 登录/注册/me 返回 createPolicy, ConnectTeamModal 预填 origin+initialUrl, RedeemInviteModal 接受链接, /join/:code 落地页(JoinPage+Login 跳回), 新建团队按钮按策略显隐, TeamManageModal 复制邀请链接; e2e 7 项过, 云库已清空待用户首注册 | schema/org.ts, repo/auth.ts, repo/org.ts, routes/team-auth.ts, teamAuth.ts, ConnectTeamModal.tsx, RedeemInviteModal.tsx, JoinPage.tsx, WorkspaceSwitcher.tsx, App.tsx, Login.tsx, TEAM_DEPLOYMENT.md | ok | ~0 |
| 23:50 | 首页头部加 WorkspaceSwitcher(原先只在测试工作页 SysHeader 有): Home.tsx home-header-actions 首位插入, 组件自包含定位可直接复用 | Home.tsx | ok | ~0 |
| 00:15 | 本地退出/切号不清中心会话的越权修复: AuthContext logout 清 teamAuth:*+lastCenterUrl+工作区重置 local; login/guest 换账号(lastLocalAccount 对比)同样清理; 根因=pony 见到 admin 团队项目与"新建团队"按钮, 系浏览器残留中心凭据(平台管理员身份)非本地数据串号(审计证明本地表均 user_id 隔离) | AuthContext.tsx, teamAuth.ts(clearAllTeamAuth) | ok | ~0 |
| 01:40 | 账户模型重构+默认admin+数据同步桥: 登录/注册页双账户类型(个人=SQLite/团队=中心库,团队账户独立登录,ProtectedRoute 认其一), migrate 空库播种 admin/admin123 平台管理员, AuthContext 增 centerUser/loginCenter/registerCenter, team-sync.ts 桥(本地账号列表/预览/打包, editor+ 权限), ImportToTeamModal 增 pick-account 模式(4步), 切换器增"同步本地账号数据"入口, export-package 抽 previewForUser/buildPackageForUser+缺表防御(修旧库 mock_endpoints_* 500); E2E: 播种/建队/列表/预览/打包/导入(新增1)/401/403 全过, 测试库已删, 真实库清回 admin+omni-parser | migrate.ts, team-sync.ts, export-package.ts, AuthContext.tsx, Login.tsx, Register.tsx, UserMenu.tsx, ProtectedRoute.tsx, ImportToTeamModal.tsx, WorkspaceSwitcher.tsx, TEAM_DEPLOYMENT.md | ok | ~0 |
| 02:10 | 切换器瘦身(用户反馈"不用嵌入这么多"): 移除 连接团队服务/邀请码常驻入口/导入本机资源/同步本地账号/断开团队, 动作区仅剩 [有中心会话?]用邀请码加入|登录团队账户 + [管理员]新建团队 + [团队态]团队管理; 数据导入/同步挪进 TeamManageModal 新"数据"页签(onOpenImport 回调, 关管理弹窗开导入弹窗); openRedeem 无凭据改跳 /login?type=team | WorkspaceSwitcher.tsx, TeamManageModal.tsx/.css | ok | ~0 |
| 03:05 | 团队模式首页改造+浏览器级E2E+CORS同源修复: Home 团队模式不拉个人dashboard(避免团队账户401噪音), 显示团队概览面板(角色/项目数/当前项目); 个人空间无本地会话时跳登录; Playwright(chrome channel)+生产服务器全旅程14/14(注册团队账户/退出清凭据/admin默认登录/建团自动进入/建项目/数据页签/同步4步/登出全清); 修复生产CORS白名单误杀同源模块脚本(origin==host剥Origin头) - 不修的话公司部署任意端口白屏 | Home.tsx/.css, WorkspaceSwitcher.tsx, index.ts | ok | ~0 |
| 03:40 | 修复会话残留bug(用户报告: 本地登录后仍见上一团队账号信息): 根因是 resetTeamSessionIfNeeded 仅在"换本地账号"时清理, 团队登录(不写LAST_LOCAL_ACCOUNT_KEY)后本地登录则条件不成立; 改为会话互斥原则 enterLocalSession/enterTeamSession - 登录哪类账户就进哪类空间并清另一类会话(团队登录保留本地token仅为显式切换); Playwright复现用户路径6/6(团队登录进团队->不退出直接本地登录->零残留->再团队登录回团队) | AuthContext.tsx | ok | ~0 |
| 04:15 | 修复团队列表内存残留(用户报告: 本地admin登录后仍见远程团队名且切换不了): 根因是 WorkspaceContext 加载 effect 的 else 分支在凭据清空后什么都不做, teams/projects React state 残留被切换器渲染; 修复为无中心会话时显式 setTeams([])/setProjects([]); Playwright复现用户4步路径6/6(团队登录进团队->点个人空间本地登录无团队名->退出再本地登录仍无->团队登录自动进团队) | WorkspaceContext.tsx | ok | ~0 |
| 04:50 | 修复团队环境保存 MySQL 语法错误: 客户端对 variables/ssl_certs 发数组([]), 团队分发器直接透传给 drizzle 空数组→SQL "near , default, default, , 30000"; 加 normalizeTextValues(数组/对象→JSON.stringify + NOT NULL文本列空值注入TEXT_DEFAULTS), create/update 均接入; 同时区分"未选项目"(400 缺少项目上下文)与"非成员"(404), 修复误导报错; 真实TiDB测试: 空数组/非空数组/更新路径全过 | team-resources.ts | ok | ~0 |
| 05:30 | 三个修复: ①团队环境表补 databases 列(schema+drizzle 0004 迁移+记账, 已应用到真实库, 本地镜像表漏列导致静默丢数据); ②环境保存按钮呼吸效果不消失+红色弹窗(客户端 res.code===200 漏了团队创建返回的201, 改为200||201 使 setDirty(false)与notification.success正确执行); ③normalizeTextValues 已在上一轮; Playwright 4/4(呼吸消失/绿色弹窗/文字保存成功); TiDB 验证 databases 往返正常 | business.ts, team-resources.ts, EnvironmentDetail.tsx, drizzle/0004+journal | ok | ~0 |
| 06:10 | 前端成功判定统一为 HTTP 状态语义(用户要求): utils/api.ts 加 is2xx/is4xx/is5xx helper; 批量替换 37 个文件 122 处写死的 code===200/201/!==200/!==201 为 is2xx(!is2xx), 自动注入 import; 折叠 res.code===200||res.code===201 为 is2xx(res.code); 修复环境保存呼吸效果+红色弹窗的同类隐患; tsc+build 通过, 无残留硬编码 | utils/api.ts + 37 客户端文件 | ok | ~0 |
| 07:20 | 修复 dev 模式 CodeMirror 多实例崩溃(用户报告: 保存必填项未填后控制台报 Unrecognized extension value + 页面空白): 根因是 Vite 预打包把 @codemirror/state 拆进两个 chunk(源码直连 import 的共享 chunk + @uiw/react-codemirror 内嵌副本), instanceof 跨实例失效; 大量 HMR 后新旧 chunk 混载触发; 修复为 vite.config.ts optimizeDeps.include 12 个 codemirror 包强制单实例; 注意 @codemirror/lang-yaml 仅存在于注释中被误加入已移除 | vite.config.ts | ok | ~0 |
| 20:50 | 修复团队 token 24h 过期后"幽灵会话"(用户报告: 刷新后 /api/team/teams、/api/tags 全部 401 刷屏; 根因: 过期后什么都不清理, centerUser 用 localStorage 陈旧 user 骗过 ProtectedRoute): api.ts 新增 handleTeamSessionExpired(清 teamAuth+退回个人空间+派发 auth-changed+3s 防抖 toast), 接入 apiFetchJSON/apiFetchCenter(仅带token时)/apiFetchBlob 三处 401 分支; AuthContext 监听 auth-changed 同步 centerUser。E2E 4 场景 11 项全过(过期无本地→登录页、过期有本地→留应用退个人空间、错密码登录不误清、有效token不受影响), 并发401只弹一次。教训: 测试时两个 dev server 抢 HMR 端口 24678 会导致页面反复重载, toast 假性消失——验证前端行为用 NODE_ENV=production 静态服务 | src/client/utils/api.ts, src/client/contexts/AuthContext.tsx | ok | ~0 |
| 21:40 | 修复误踢(用户报告: 团队登录后点测试类型跳回登录页): 复现发现 /api/dashboard/* 是本地路由(本地JWT验证), 团队模式下被路由到中心且带团队token → 401 'Authentication required' 被昨天的过期拆解逻辑误判为会话过期 → 清会话踢登录页。修复: 拆解改为精确判定——仅当 401 报文含'团队'(需要团队登录/团队登录已失效)才拆解; apiFetchBlob 移除拆解(blob无法廉价区分); dashboard 401 由页面 catch 静默降级为零值。E2E 5 场景全过(用户流程+真过期A/B+错密码C+有效D) | src/client/utils/api.ts | ok | ~0 |
| 22:30 | 列表页"撑满不滚动"重构(用户反馈: 10行+大片空白/末行截半出滚动条/标签列撑宽): ①useViewportPageSize 重写为真实测量(挂到 .alist-table-wrap, ResizeObserver+MutationObserver 实测 thead/行高/容器高, 替代 innerHeight-310 硬编码); ②9个列表页接入 ref + 去掉 setPageSize(res.data.pageSize) pinning + pageSize 变化重拉取; ③fetch/load 全部加竞态守卫(StrictMode+校准重拉并发时过期响应覆盖新响应, 表现为12行钉死); ④table-layout:fixed + 全列 td 省略号截断, 每页补 th 列宽(标签180/日期150/操作110-200), 横向滚动条消失; ⑤MockList wrap 补 flex:1; ⑥ScheduleList 翻页默认带上已应用筛选。浏览器实测: 5页 scrollHeight==clientHeight 三维零滚动, 小屏8行/大屏16行自动适配, 手动50条 resize 不覆盖、刷新恢复自动 | useViewportPageSize.ts, 9个列表页, tables.css, MockList.css, ScenarioList.css, AppList.tsx | ok | ~0 |
| 23:50 | 列表页三卡连体(用户要求: 筛选/表格/分页边框贴接+多展示一行): 定位到间隙真凶是页面 CSS 全局泄漏——CaseSetDetail.css 死代码 .alist-pagination{margin-top:1rem} 和 MockList.css 无作用域复制的整套 .alist-* 体系(margin-bottom 0.75rem 等), Vite 页面 CSS 全局生效泄漏到所有列表页; 修复: 删死代码、MockList.css 重写为纯 .mock-list-root 作用域(sset-btn-create 迁 empty.css); 共享 filters/tables/pagination.css 实现连体卡片组(筛选卡上圆角无下边框/表格区中段 :has(+.alist-pagination) 去底边/分页条尾段下圆角, 全组单层 1px 边框), filter padding 16→12、分页 12→8。实测: 间隙 0×0、接缝单 1px、810px 屏 11→12 行、700px 屏 8→9 行、Mock/Web/Schedule/AppList(无分页变体保留底边框) 四页零滚动 | MockList.css, CaseSetDetail.css, filters.css, tables.css, pagination.css, empty.css | ok | ~0 |
| 2026-08-31 23:59 | 列表页列宽全固定(用户要求: 勾选/名称/URL/标签/时间/操作列各自定宽): 先浏览器实测各列最小内容宽度(勾选50/日期149/操作120-204 等), 再按"每表只留一个弹性列(无 width), 其余全部固定 px"方案落 10 个页面——弹性列=URL(ApiList)/名称(Web/Pc/Mobile/Set/Schedule)/描述(Scenario)/路径(Mock)/包名(AppList), 吸收屏宽差异保证任何宽度零横向滚动; 刚性列按实测+余量: 勾选52(原36会裁掉复选框)/时间152/操作96-204。实测 1440+1280 两档: 刚性列宽度完全不变、无横竖滚动、名称/URL 超长省略+title | 10 个列表页 tsx | ok | ~0 |
| 2026-08-31 24:10 | 表格"恰好填满"收尾(用户截图指出表格底部仍有空白): ①floor 取整余数改为行高微拉伸 — useViewportPageSize 满页时设 --row-h=(可用高-1px)/行数(每行+0.4~3px,上限+8px), tables.css 加 tbody tr{height:var(--row-h,auto)}; 行数不足/手动选大页保持自然行高。②排障中发现两个结构性 bug: (a)观察器在 callback ref 里创建、在 effect cleanup 里 disconnect, StrictMode 挂载→清理→重挂后 callback ref 不重跑, 观察器被杀无人重建 → 改为容器元素存 state、观察器完全由 effect 拥有; (b)resize 路径走 rAF, 后台/遮挡标签页 rAF 被浏览器暂停永不执行 → 改同步 compute。③IAB 标签页 guest 崩溃后会假死(MO活/resize死/点击超时/截图失败), 换新标签页即恢复 — 排障时先怀疑环境。最终: 810/747/1050 三档=12/10/17行, 底部空白≤0.5px 零滚动, 末页7行不拉伸, 手动50条自然行高+表内滚动 | useViewportPageSize.ts, tables.css | ok | ~0 |
| 2026-08-31 24:40 | 每页条数下拉重构(用户要求: 底部应向上展开 + 选项按自动行数计算 + 可自定义): ①FormSelect 加自动方向翻转 — 打开时测 trigger 底部空间, 不够放下拉(选项数*33px, 上限240)则加 flip-up 类向上展开(absolute 模式 bottom:100%, fixed 模式 top 反算), 分页条贴屏底不再被 .alist overflow:hidden 裁切; 顶部筛选区空间充足仍向下。②新共享组件 PageSizeSelect — 选项=autoSize 的 1~4 倍(满屏/2屏/3屏/4屏, 钳制5~100)+当前值去重排序, FormSelect 扩展 allowCustom/onCustomSubmit 在下拉顶部渲染自定义输入行(回车/确定生效); 9 个列表页分页条统一替换。实测: 向上展开且在视口内、810屏选项[12/24/36/48]、输入30回车→30条/页、选回12恢复恰好填满 | FormSelect.tsx/.css, PageSizeSelect.tsx(新), 9 个列表页 | ok | ~0 |
| 2026-08-31 24:55 | 顶栏形状统一(用户要求: 环境切换器和头像改方形圆角): Layout.css .hdr-env-btn 胶囊(999px, padding 4/10, 高26)改 height:32+padding 0 12+radius var(--radius-sm)=6px, 与旁边 32px 主题按钮高度/圆角一致; user-menu.css 头像/hover环 50%→8px; Header.css 主页 .hdr-avatar/.hdr-avatar-ph 50%→8px 同步(两处头像入口统一)。状态小圆点(hdr-env-dot/ws-mode-dot)、开关旋钮、步骤序号圆保持圆形不动。实测 computed style: env 6px/32h, avatar 8px, themeBtn 6px/32h | Layout.css, user-menu.css, Header.css | ok | ~0 |
